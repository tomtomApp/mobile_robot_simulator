...
class pure_pursuit():
	def __init__(self):
		rospy.init_node('pure_pursuit2023_estimated', anonymous=True)
		...
		self.odom_sub = rospy.Subscriber("/estimated_pose", Odometry, self.odom_callback)
		rospy.Subscriber("/path_num", Int32, self.path_num_callback)
	...
	def path_follower(self):
		if self.path_first_flg == True and self.odom_first_flg == True and self.imu_first_flg == True and self.path_num_first_flg == True and len(self.path) != 0:
			if self.start == 0:
				self.start += 1
			...
			self.laterY = self.current_y
			self.theta = self.current_yaw_euler
			self.theta = self.radian_Normalization(self.theta) - self.intialTheta
	...
	def cb_get_path_topic_subscriber(self,msg):
		if self.path_first_flg != True and self.path_num != 0:
	...
	def odom_callback(self, msg):
		...
		self.current_y = msg.pose.pose.position.y
		self.odom_first_flg = True
		print('eskf:', self.current_x, self.current_x)
		
	def path_num_callback(self, msg):
		if self.path_num_first_flg == False:
			self.path_num = msg.data;
			print('self.path_num:',self.path_num)
			self.path_num_first_flg = True;
